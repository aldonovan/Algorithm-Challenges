...
class DSU {
 public: 
    DSU(int n) : parents(n + 1, 0), number_of_nodes(n + 1, 1), is_valid(n + 1, false) {
        for (int i = 1; i <= n; ++i) {
            parents[i] = i;
        }
    }
    ...
    bool get_validity (int vertex) {
        return is_valid[find_parent(vertex)];
    }
    void set_validity (int vertex, bool validity) {
        is_valid[find_parent(vertex)] = validity;
    }
    bool solve() {
        return get_validity(1);
    }
 private: 
    vector<int> parents;
...
int main() {
    // For any connected component adjacent, it must have all nodes less than or equal to curr
    // It either is navigable, in which case curr node is reachable if <= 1 + size.
    // Or it is not navigable, in which case it will still not be navigable when adding new node. 
    // If at least one neighbor is navigable, they all become navigable 
    int t;
    cin >> t;
    while (t--) {
        int n, m;
        ...
        for (const auto& [danger, index] : dangers) {
            ...
            for (int neighbor : edges[index]) {
                ...
                if (vertices.get_validity(neighbor) && vertices.get_number_of_nodes(neighbor) >= danger) {
                    now_valid = true;
                    break;
                }
            }

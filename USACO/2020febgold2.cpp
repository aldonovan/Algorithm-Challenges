...
int main() {
    ...
    // For each starting point, count number of subsets where that endpoint is start of an interval
    // This is 2^(# of segments that start after current segment)*2^(# of segments end before start)
    // 2^(n - 1 - #segments before that intersect start) -> can use binary indexed tree
